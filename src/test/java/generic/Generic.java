...
class Generic {
    ...
    void mission5() {
        ...
        final SimpleList<LaserPrinter> laserPrinters = new SimpleArrayList<LaserPrinter>(
                laserPrinter);
        final SimpleList<SuperLaserPrinter> superLaserPrinters = new SimpleArrayList<>(
                superLaserPrinter);

        SimpleList.copy(laserPrinters, printers);
        SimpleList.copy(superLaserPrinters, printers2);

        Assertions.assertThat(printers.get(0)).isSameAs(laserPrinter);
        Assertions.assertThat(printers2.get(0)).isSameAs(superLaserPrinter);
    }
